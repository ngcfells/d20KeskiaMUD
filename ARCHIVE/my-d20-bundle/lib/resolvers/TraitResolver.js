...
class TraitResolver {
  ...
  static _applySingleTrait(state, player, trait) {
    ...
    //
    // 2) Physiology / modern trait system
    //
    const stateBlock = trait.state || {};
    const modifiers = trait.modifiers || {};

    // Apply state block directly to metadata
    ...
    if (modifiers.attributes) {
      const current = player.getAttributes(); // must be a plain object or similar
